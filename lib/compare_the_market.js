    ...
    create_puppeteer(config.puppeteer)
        .then( async browser => {
            ...
            if (config.output.detail === 'simple') {
                data = data.map(q => `${q.name} - ${q.price}${(q.telematics)?' - Telematics':''}`);
            } else {
                data = data.map(q => `${q.name} - ${q.price}${(q.telematics)?' - Telematics':''} - E: [${q.excesses.join(' | ')}]`);
            }
            ...
            if (config.output.format === 'text') {
                ...
                let carinfo = `Reg: ${config.reg} - ${date}`;
                data = data.join('\n');
